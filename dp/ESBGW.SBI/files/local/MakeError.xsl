...
<xsl:stylesheet version="1.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
	...
	<xsl:template match="/">
		...
		<xsl:variable name="vError">
			<xsl:copy-of select="dp:transform('local:///sbi/maps/CopyAll.Err.xsl', $vMsg)" />
		...
		<xsl:variable name="vMQXQHbinTemp">
			<xsl:variable name="vMQXQH" select="dp:variable('var://context/msg/xmit')" />
			<xsl:choose>
				<xsl:when test="contains($vMQXQH,$FakeManagerCZ)"><xsl:value-of select="substring-before($vMQXQH,$FakeManagerCZ)"/><xsl:value-of select="$ManagerCZ"/><xsl:value-of select="substring-after($vMQXQH,$FakeManagerCZ)"/></xsl:when>
				<xsl:when test="contains($vMQXQH,$FakeManagerM99)"><xsl:value-of select="substring-before($vMQXQH,$FakeManagerM99)"/><xsl:value-of select="$ManagerM99"/><xsl:value-of select="substring-after($vMQXQH,$FakeManagerM99)"/></xsl:when>
				<xsl:otherwise>
					<xsl:value-of select="$vMQXQH"/>
				</xsl:otherwise>
			</xsl:choose>
		</xsl:variable>
		
		<xsl:variable name="vMQXQHbinTempLast">
			<xsl:variable name="vMQXQH" select="dp:variable('var://context/msg/xmit')" />
			<xsl:choose>
				<xsl:when test="contains($vMQXQHbinTemp,$FakeManagerCZ)"><xsl:value-of select="substring-before($vMQXQHbinTemp,$FakeManagerCZ)"/><xsl:value-of select="$ManagerCZ"/><xsl:value-of select="substring-after($vMQXQHbinTemp,$FakeManagerCZ)"/></xsl:when>
				<xsl:when test="contains($vMQXQHbinTemp,$FakeManagerM99)"><xsl:value-of select="substring-before($vMQXQHbinTemp,$FakeManagerM99)"/><xsl:value-of select="$ManagerM99"/><xsl:value-of select="substring-after($vMQXQHbinTemp,$FakeManagerM99)"/></xsl:when>
				<xsl:otherwise>
					<xsl:value-of select="$vMQXQHbinTemp"/>
				</xsl:otherwise>
			</xsl:choose>
		</xsl:variable>
		
		<xsl:variable name="vMQXQHbin" select="$vMQXQHbinTempLast"/>
