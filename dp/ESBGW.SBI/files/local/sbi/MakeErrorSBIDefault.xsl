...
<xsl:stylesheet version="1.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
	...
	<xsl:template match="/">
		...
		<xsl:variable name="vStatus" select="XslFunc:getStatusFromContext(dp:variable('var://context/input/InputMesType'))"/>
		...
		<xsl:call-template name="XslFunc:MakeError">
			...
			<xsl:with-param name="pMT" select="namespace-uri($vMsg/node()/*[1])"/>
			<xsl:with-param name="pCA" select="namespace-uri($vMsg/node()/*[1])"/>
