...
<!-- Шаблон для отправки сообщения о событии в очередь сообщений -->
	...
	********************************
	...
	<xsl:template name="XslFunc:MakeError">
		...
		<xsl:variable name="vRootTagName" select="XslFunc:GetResponseName(local-name($pNode/node()))"/>
		<xsl:element name="{$vRootTagName}" namespace="{namespace-uri($pNode/node())}">
		...
		<xsl:variable name="vStatus">
			<xsl:choose>
				<xsl:when test="starts-with($mqrc, '2') and string-length($mqrc)= 4">
					<xsl:choose>
						<xsl:when test="$mqrc='2059'">
							<xsl:copy-of select="$diag/diag/status[@name='MQRC2059']"/>
						</xsl:when>
						<xsl:when test="$mqrc='2009'">
							<xsl:copy-of select="$diag/diag/status[@name='MQRC2009']"/>
						</xsl:when>
						<xsl:when test="$mqrc='2010'">
							<xsl:copy-of select="$diag/diag/status[@name='MQRC2010']"/>
						</xsl:when>
						<xsl:when test="$mqrc='2053'">
							<xsl:copy-of select="$diag/diag/status[@name='MQRC2053']"/>
						</xsl:when>
						<xsl:when test="$mqrc='2218'">
							<xsl:copy-of select="$diag/diag/status[@name='MQRC2218']"/>
						</xsl:when>
						<xsl:when test="$mqrc='2025'">
							<xsl:copy-of select="$diag/diag/status[@name='MQRC2025']"/>
						</xsl:when>
						<xsl:when test="$mqrc='2012'">
							<xsl:copy-of select="$diag/diag/status[@name='MQRC2012']"/>
						</xsl:when>
						<xsl:otherwise>
