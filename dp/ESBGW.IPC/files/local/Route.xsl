...
<xsl:stylesheet version="1.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
	...
	<xsl:template match="/">
		...
		<xsl:variable name="TestvMQXQH" select="dp:parse(dp:request-header('MQXQH'))"/>
		<xsl:variable name="vMQXQH">
			<MQXQH>
				<xsl:for-each select="$TestvMQXQH/MQXQH/node()">
					<xsl:choose>
						<xsl:when test="local-name(.) = 'RemoteQMgrName' and contains(./text(), '.MCP')"><RemoteQMgrName><xsl:value-of select="substring-before(./text(), '.MCP')"/></RemoteQMgrName></xsl:when>
